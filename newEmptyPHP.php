...
<div id="nav">
  ...
  <input type="button"  name="submit" class='tfbutton' onclick="location.href='locations.php'" value="Next"/><br/>
         <br/><br/>
   <input type="button"  name="submit" class='navbutton' onclick="location.href='admin.php'" value="Page Admin login"/><br/>
  <input type="button"  name="submit" class='navbutton' onclick="location.href='create.php'" value="Directory management"/><br/>
  <input type="button"  name="submit" class='navbutton' onclick="location.href='locations.php'" value="Database management"/><br/>
...
if(empty($_GET['chmod']))
    {
$errorMsg.='Could not move the file.<a href=create.php>Back to main page</a>';
{
    

	if(is_dir($rootdir.'/'.$_GET['dir'].'/'.$_GET['file']))
	{
		@chmod($rootdir.'/'.$_GET['dir'].'/'.$_GET['file'], octdec($_GET['chmod'])) or error('Could not CHMOD the file.<a href=create.php>Back to main page</a>');
	}
	else
	{
...
if(!isset($_GET['edit']))
{
	...
	if(str_replace('//', '', $dir) != '' && str_replace('//', '', $dir) != '/')
		echo '<a href="create.php?dir='.$up1dir.'"><img src="images/up.png" alt="Up" width="25"/>Up 1 Directory</a> | <a href="create.php"><img src="images/folder_home.png" alt="Home" width="25"/>Home</a><br><br>';
...
<tr><th>#</th><th>Select</th><th width="36">Icon</th><th>Name</th><th>Function</th><th>Permissions</th><th>Last Modified</th>';
	for($i = 0; $i< count($files); $i++)
	{
		...
		if(is_dir($rootdir.'/'.$dir.'/'.$file))
			echo '<tr><td>'.$filenum.'</td><td><input type="radio" name="file" value="'.$file.'"/></td width="36"><td><img src="types/folder.png" alt="Type"/></td><td><a href="create.php?cd='.$dir.'/'.$file.'">'.$file.'</a></td><td><input type="submit" class="delbutton" name="delete" value="Delete" onclick="return confirm("Are you sure yo want to delete?")" /></td><td>'.$perms.'</td><td>'.$mtime.'</td></tr>';
		else
